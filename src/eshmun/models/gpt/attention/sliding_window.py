...
class SlidingWindowAttention(BaseAttentionModule):
    def __init__(self, config: EshmunGPTConfig):
        super().__init__(config)
        
        assert config.window_size > 0, "window_size should be a positive integer, got: {}".format(config.window_size)
    ...
    def _build_attention_mask(self, attention_mask: torch.Tensor):
        L = attention_mask.shape[-1]

        window = torch.tril(torch.ones(L, L), diagonal=-(self.window_size + 1))
        window = window.to(dtype=attention_mask.dtype).to(attention_mask.device)
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor | None = None):
        ...
        if attention_mask is not None:
            scores = scores + self._build_attention_mask(attention_mask)
